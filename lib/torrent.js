...
var Torrent = function(client, clientId, port, file, downloadPath, peerLimit) {
  ...
  this.peerPool = new PeerPool("Torrent", 1, client.peerPool);
  ...
  //  BANDWIDTH
  /////////////////////////////////////
  this.maxBandwidthUpload = 512 * 1024;
  ...
  setInterval(function(){ assignUploadBandwidth(self) }, 5e3);
...
Torrent.prototype.addPeer = function(peer) {
  if (!(peer.getIdentifier() in this.peers)) {
    ...
    console.log("addPeer", peer.getIdentifier());
...
//  BANDWIDTH
////////////////////////////////
function assignUploadBandwidth(self){
  var nPeers = Object.keys(self.peers).length;
  if (nPeers){
    var busyPeers = [];

    //Look for any peers that are have been uploading anything.
    for (var id in self.peers){    
      // console.log("id", id);
      // console.log("upThisSec", self.peers[id].upThisSec);
      // console.log("allowance", self.peers[id].uploadAllowance);
      // console.log("currentUploadRate", self.peers[id].currentUploadRate);
      if (self.peers[id].currentUploadRate > 0){
        busyPeers.push(id);
      }else{
        self.peers[id].uploadAllowance = Piece.CHUNK_LENGTH; 
      }
    }

    var maxPeers;
    maxPeers = nPeers = busyPeers.length;
    //Divide bandwidth allowance among active peers
    var bandwidthPerPeer = (nPeers) ? self.maxBandwidthUpload / nPeers : Piece.CHUNK_LENGTH;
    if (bandwidthPerPeer < Piece.CHUNK_LENGTH){
      maxPeers = self.maxBandwidthUpload / Piece.CHUNK_LENGTH;
      bandwidthPerPeer = Piece.CHUNK_LENGTH;
    }

    var count = 0;
    for (var i = 0; i < busyPeers.length; i++){
      var id = busyPeers[i];
      if (count <= maxPeers){
        console.log("assigning " + bandwidthPerPeer + " to " + id);
        self.peers[id].uploadAllowance = bandwidthPerPeer;
      }else{
        console.log("assigning " + Piece.CHUNK_LENGTH + " to " + id);
        self.peers[id].uploadAllowance = Piece.CHUNK_LENGTH; 
      }
      count++;
    }
    LOGGER.info('Assigned ' + bandwidthPerPeer + ' to each peer.', nPeers, self.maxBandwidthUpload);    
  }
}
...
function trackerUpdated(self, tracker, data) {
  ...
  //LOGGER.info(util.format('[0] Current #peers = %s', _.size(self.peers)));
      ...
      for (var i = 0; i < data.peers.length; i++) {
        ...
        if (!self.peers[Peer.getIdentifier(peer)]) {
          if (self.peerPool.requestSlot()) {
            ...
            peerObj.on(Peer.DISCONNECT, function () {
            ...
            var peerObj = new Peer(peer.peer_id, peer.ip, peer.port, self);
            peerObj.on(Peer.CONNECT, function () {
  ...
  //LOGGER.info(util.format('[2] Current #peers = %s', _.size(self.peers)));
  self.emit('updated');
}
