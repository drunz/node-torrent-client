...
var Torrent = function(client, clientId, port, file, downloadPath, peerLimit) {
  ...
  this.peerPool_ul = new PeerPool("Torrent upload",   client.peerPool_ul, 1, 1);
  this.peerPool_dl = new PeerPool("Torrent download", client.peerPool_dl, 1, 1);
...
Torrent.prototype.handleConnection = function(peer) {
  ...
  if (self.peerPool_ul.requestSlot()) {
...
function tryPeer(self, peer) {
  ...
  var lowest = self.peerPool_dl.discard();
  ...
  if (lowest) {
    ...
    if (score > lowestScore) {
      ...
      peer.choke();
      self.peerPool_dl.add(lowest); // add back and trigger pool update
    }
  }
}
    ...
    if (!self.isComplete && data.peers) {
      for (var i = 0; i < data.peers.length; i++) {
        ...
        if (!self.peers[Peer.getIdentifier(peer)]) {
          if (self.peerPool_dl.requestSlot()) {
            ...
            peerObj.on(Peer.DISCONNECT, function () {
              self.peerPool_dl.releaseSlot();
            });
            self.peerPool_dl.add(peerObj);
            // self.addPeer(peer); // TODO: stop passing full torrent through to peer
          }
          else if (self.usePeerSelection && self.peerPool_dl.requestOverflowSlot()) {
            LOGGER.info(util.format('Too many peers (%s/%s), checking if %s:%s is any good', 
              _.size(self.peers), self.peerPool_dl.peerLimit, peer.ip, peer.port));
            ...
            peerObj.on(Peer.DISCONNECT, function () {
              self.peerPool_dl.releaseOverflowSlot();
            });
          }
          else {
