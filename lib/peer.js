...
var Peer = function(/* stream */ /* or */ /* peer_id, address, port, torrent */) {
  ...
  this.uploadAllowance = 1024 * 1024;
  this.maxBandwidthDownload = 1024 * 1024 * 1024;
  ...
  setTimeout(function() { forceUpdateRates(self) }, 1e3);

  setTimeout(function() { resetBandwidth(self) }, 1000);
...
Peer.prototype.getScore = function() {
  var self = this;

  console.log("** SCORE ** up %s", self.uploadRates);
  console.log("** SCORE ** down %s", self.downloadRates);
  console.log("** SCORE ** up avg %s", self.currentUploadRate);
  console.log("** SCORE ** down avg %s", self.currentDownloadRate);

  if (self.bitfield) {
    var rate     = self.interested ? self.currentUploadRate / _.max(self.uploadRates) : 1;
    var progress = self.bitfield.cardinality() / self.bitfield.length;
    var score    = (rate+progress) > 0 ? (2 * rate * progress) / (rate + progress) : 0;
    //var rate = self.currentDownloadRate;
    LOGGER.info('Peer rate:     %s', rate);
    LOGGER.info('Peer progress: %s', progress);
    LOGGER.info('Peer score:    %s', score);
    return score;
  }
  else {
    LOGGER.info('Couldn\'t rate peer');
    return 0;
  }
};
...
Peer.prototype.choke = function() {
  //this.sendMessage(new Message(Message.CHOKE));
  this.disconnect("Above peer limit, have better peers in pool");
};
...
function onData(self, data) {
  ...
  if (self.downThisSec >= self.maxBandwidthDownload){
    console.log("throttling", self.downThisSec, self.maxBandwidthDownload);
    self.stream.pause();
    setTimeout(function(){self.stream.resume()}, 10);
  }else{
    self.downThisSec += data.length;
...
function sendData(self) {
  ...
  (function next() {
    if (self.toSend.length > 0) {
      ...
      if (self.upThisSec >= self.uploadAllowance){
        setTimeout(function(){ProcessUtils.nextTick(next)}, 10);
      }
      else {
...
// 
                ...
                case Message.PIECE:
                  ...
                  // LOGGER.info('Peer [' + self.getIdentifier() + '] got piece ' + index + ', begin ' + begin + ', length ' + data.length);
                  ...
                  if (self.requests[index] && self.requests[index][begin]) {
                    ...
                    updateRates(self, 'down');
                    LOGGER.info('Downloading piece: ' + Peer.getIdentifier(self) + ', download rate = ' + (self.currentDownloadRate / 1024) + 'Kb/s');
                  }
