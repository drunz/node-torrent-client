...
/**
 * The functions below are used by the File object for read/write operations
 * instead of the fs ones. We impose a maximum number of file descriptors opened at one time,
 ...
 * is reached and a File that doesn't have an opened descriptor requests access, the least
 * recently used file descriptor gets closed to make space for the requesting file.
  ...
  if (file.fd) {
    ...
    if (fdcount>=MAXOPEN) {
      ...
      _closelru(function(err) {
       ...
       // we're under the fd limit, open a new one
      _open();
    }
  }
}
