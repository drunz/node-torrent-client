...
function announce(self, event, cb) {
  ...
  LOGGER.info('torrent ' + self.torrent.name + ' announce' + (event ? ' ' + event : ''));

  var handlerClass = protocol[self.url.protocol];
	var compactMode = (self.url.protocol === 'udp');

  if (handlerClass) {
    var handler = new handlerClass();
    var data = self.torrent.trackerInfo();
    self.state = CONNECTING;
    handler.handle(self, data, compactMode, event, function(info, error) {
      if (error) {
        LOGGER.warn('torrent ' + self.torrent.name + ' announce'+ (event ? ' ' + event : '') + ' error from ' + self.url.href + ': ' + error.message);
        self.state = ERROR;
        self.errorMessage = error.message;
        if (event === 'started') {
          LOGGER.warn('torrent ' + self.torrent.name + ' retry announce \'started\' in ' + ANNOUNCE_START_INTERVAL + 's');
          setTimeout(function() {
