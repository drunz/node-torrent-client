...
HTTP.prototype = {
  ...
  handle: function(tracker, data, compact, event, callback) {
      ...
      res.on('end', function() {
        ...
        if (!timedOut) {
          ...
          if (res.statusCode === 200) {
            ...
            self._parseResponse(response, compact);
          } else {
          ...
          self._complete(null, new Error('request timed out'));
        }
      });
    });
  ...
  _parseResponse: function(response, compact) {
    ...
    if (response['failure reason']) {
      ...
      if (response.peers) {
        LOGGER.debug('Parsing peers, compact = ' + compact);
				if (compact || typeof(response.peers) === 'string') {
